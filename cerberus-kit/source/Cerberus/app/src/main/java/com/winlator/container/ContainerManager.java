...
import com.winlator.cerberus.runtime.CerberusBionicContainerPattern;
...
public class ContainerManager {
    ...
    /** Activates the same physical Cerberus container inside an alternate root (Bionic). */
    public void activateContainer(Container container, RootFS selectedRootFS) {
        activateContainer(container);
        if (selectedRootFS == null || selectedRootFS.getRootDir().equals(RootFS.find(context).getRootDir())) return;
        File selectedHome = new File(selectedRootFS.getRootDir(), "home");
        if (!selectedHome.isDirectory()) selectedHome.mkdirs();
        File link = new File(selectedHome, RootFS.USER);
        FileUtils.delete(link);
        FileUtils.symlink(container.getRootDir().getAbsolutePath(), link.getAbsolutePath());
    }
    ...
    private Container createContainer(JSONObject data) {
        try {
            ...
            CerberusRuntimeProfile runtimeProfile = CerberusContainerRuntime.read(container);
            boolean patternOk = runtimeProfile.isBionic()
                    ? CerberusBionicContainerPattern.extract(context, runtimeProfile, containerDir)
                    : extractContainerPatternFile(container.getWineVersion(), containerDir);
            if (!patternOk) {
    ...
    private void duplicateContainer(Container srcContainer) {
        ...
        dstContainer.setExtraData(srcContainer.getExtraDataCopy());
