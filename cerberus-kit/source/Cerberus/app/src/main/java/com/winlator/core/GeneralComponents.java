...
public abstract class GeneralComponents {
    ...
    private static File findFileRecursive(File directory, String filename) {
        if (directory == null || !directory.isDirectory() || filename == null || filename.isEmpty()) return null;
        File[] files = directory.listFiles();
        if (files == null) return null;
        for (File file : files) if (file.isFile() && filename.equals(file.getName())) return file;
    ...
    private static File findAdrenoManifest(File source) {
        if (source == null || !source.isDirectory()) return null;
        File[] files = source.listFiles();
        if (files == null) return null;
        for (File file : files) if (file.isFile() && file.getName().toLowerCase(Locale.ENGLISH).endsWith(".json")) return file;
        for (File file : files) if (file.isDirectory()) {
            File found = findAdrenoManifest(file);
    ...
    public static String getDefinitivePath(Type type, Context context, String identifier) {
        ...
        if (type == Type.SOUNDFONT && isBuiltinComponent(type, identifier)) {
            ...
            File manifestFile = findAdrenoManifest(source);
            if (manifestFile == null) return null;
            try {
                JSONObject manifestJSONObject = new JSONObject(FileUtils.readString(manifestFile));
                String libraryName = manifestJSONObject.optString("libraryName", "").trim();
                if (libraryName.isEmpty()) return null;
                File libraryFile = new File(manifestFile.getParentFile(), libraryName);
                if (!libraryFile.isFile()) libraryFile = findFileRecursive(source, libraryName);
                return libraryFile != null && libraryFile.isFile() ? libraryFile.getPath() : null;
            }
    ...
    public static void openFileForInstall(final MainActivity activity, final Type type, final Spinner spinner, final String defaultItem) {
    ...
    public static void showDownloadableListDialog(Type type, final Spinner spinner, final String defaultItem) {
        final Activity activity = (Activity)spinner.getContext();
