...
import com.winlator.cerberus.runtime.CerberusBionicInstaller;
import com.winlator.cerberus.runtime.CerberusContainerRuntime;
import com.winlator.cerberus.runtime.CerberusRuntimePreflight;
...
import com.winlator.xenvironment.components.CerberusBionicProgramLauncherComponent;
...
public class XServerDisplayActivity extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    private CerberusRuntimeProfile cerberusRuntimeProfile;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (!isGenerateWineprefix()) {
            ...
            if (container == null) {
                AppUtils.showToast(this, "Container Cerberus não encontrado.");
                finish();
                return;
            }
            cerberusRuntimeProfile = CerberusContainerRuntime.read(container);
            if (cerberusRuntimeProfile.isBionic()) {
                if (!CerberusBionicInstaller.ensureInstalled(this)) {
                    AppUtils.showToast(this, "Bionic Engine ausente. Execute PREPARE-BIONIC antes do build.");
                    finish();
                    return;
                }
                rootFS = RootFS.findBionic(this);
            }
            containerManager.activateContainer(container, rootFS);

            boolean wineprefixNeedsUpdate = !cerberusRuntimeProfile.isBionic() && container.getExtra("wineprefixNeedsUpdate").equals("t");
            ...
            wineInfo = cerberusRuntimeProfile.isBionic() ? WineInfo.MAIN_WINE_INFO : WineInfo.fromIdentifier(this, wineVersion);

            if (!cerberusRuntimeProfile.isBionic() && wineInfo != WineInfo.MAIN_WINE_INFO) rootFS.setWinePath(wineInfo.path);
        ...
        Executors.newSingleThreadExecutor().execute(() -> {
            if (!isGenerateWineprefix()) {
                if (cerberusRuntimeProfile == null || !cerberusRuntimeProfile.isBionic()) {
                    setupWineSystemFiles();
                    extractGraphicsDriverFiles();
                    changeWineAudioDriver();
                }
                else {
                    String error = CerberusRuntimePreflight.check(this, container, cerberusRuntimeProfile);
                    if (error != null) {
                        AppUtils.showToast(this, "CERBERUS PREFLIGHT: " + error);
    ...
    private void setupXEnvironment() {
        ...
        GuestProgramLauncherComponent guestProgramLauncherComponent = null;
        CerberusBionicProgramLauncherComponent bionicProgramLauncherComponent = null;
        boolean useBionic = container != null && cerberusRuntimeProfile != null && cerberusRuntimeProfile.isBionic();
        if (useBionic) bionicProgramLauncherComponent = new CerberusBionicProgramLauncherComponent(cerberusRuntimeProfile);
        else guestProgramLauncherComponent = new GuestProgramLauncherComponent();
        ...
        if (container != null) {
            ...
            if (useBionic) bionicProgramLauncherComponent.setGuestExecutable(guestExecutable);
            else guestProgramLauncherComponent.setGuestExecutable(guestExecutable);

            envVars.putAll(container.getEnvVars());
            if (shortcut != null) envVars.putAll(shortcut.getExtra("envVars"));
            if (!envVars.has("WINEESYNC")) envVars.put("WINEESYNC", "1");

            if (!useBionic) guestProgramLauncherComponent.setBox64Preset(shortcut != null ? shortcut.getExtra("box64Preset", container.getBox64Preset()) : container.getBox64Preset());
        }
        ...
        if (useBionic) {
            ...
            bionicProgramLauncherComponent.setTerminationCallback((status) -> exit());
